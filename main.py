...
# location of data relative to code base path
fd_data = os.path.join("Data", "5H1AI_logs")
fn_data = [x for x in os.listdir(fd_data) if x.find(
    'sample_game') > -1]  # list of data file names (log files)
...
def meta_game_stats(games_f, print_f=True, long_print_f=False):
    ...
    for g_num, g in games_f.items():
        ...
        if len(g.error_hands) > 0:
            ...
            if long_print_f:
                print("%d hands missing information for game %s" %
                      (len(g.error_hands), g_num))
        if g.start_hand > 0:
        ...
        if g.combine_player_diff is not None:
            games_with_combined_mismatch_players.update(
                {g_num: g.combine_player_diff})
            if long_print_f:
                print("%s combined mismatch players for game %s" %
                      (g.combine_player_diff, g_num))

    if print_f:
        print("Number of games processed (game object instantiations): %d" %
              len(games_f))
        print("Number of games with hand parse errors: %d" %
              len(games_with_file_parse_errors))
        print("Number of games with hand calculation errors: %d" %
              len(games_with_error_hands))
        print("Number of games with starting hand number > 0: %d" %
              len(games_start_hand_not_zero))
        print("Min %d number of hands in game %s" %
              (min_number_hands_played[1], min_number_hands_played[0]))
        print("Max %d number of hands in game %s" %
              (max_number_hands_played[1], max_number_hands_played[0]))
        print("Number of games with hand number missing in sequence: %d" %
              len(games_missing_hands))
        print("Number of combined games with mismatch players: %d" %
              len(games_with_combined_mismatch_players))
...
def create_player_df(player_f):
    def calc_hand_shift_vars(df_ff, shift_var_name_ff):
        ...
        t_df_ff = df_ff[['hand', shift_var_name_ff]].sort_values(
            by='hand', ascending=True).reindex()
        if all((df_ff['hand'] - df_ff['hand'].shift(1))[1:] == 1):
            t_df_ff['prev_' +
                    shift_var_name_ff] = t_df_ff[shift_var_name_ff].shift(1)
        else:
    ...
    df_f = pd.DataFrame(data=t_records).sort_values(
        by=['game', 'hand'], ascending=True)

    # temp df of shift variables
    t_df_f = df_f.groupby('game').apply(
        calc_hand_shift_vars, shift_var_name_ff='outcome').reset_index()
...
def create_base_df_wrapper(players_f):
    ...
    print('%d players in data set: %s' %
          (len(df.player.unique()), df.player.unique()))
    print('Min number of hands per game: %d' %
          df.groupby('game').hand.nunique().min())
    print('Max number of hands per game: %d' %
          df.groupby('game').hand.nunique().max())
    ...
    df['relative_start_stack'] = df['start_stack'] / \
        df.groupby(['game', 'hand'])['start_stack'].transform('max')
    df['rank_start_stack'] = df.groupby(['game', 'hand'])[
        'start_stack'].rank(method='max', ascending=False)
...
print("Test statistic z = (p1 - p2)/stdev and p1 is %s is True" %
      (sample_partition_binary_field))
print(df_prop_test)
...
X_col_name = ['any_blind', 'prev_outcome_loss', 'slansky_rank',
              'rank_start_stack']     # chen_rank, slansky_rank, premium_hole
y_col_name = ['preflop_fold']
# base field name: value to drop for identification; if value is None, then no dummies are dropped
dummy_vars = {'player': 'Pluribus'}
add_const = False

# process dummy vars
if len(dummy_vars) > 0:
    df_logistic = df[X_col_name +
                     list(dummy_vars.keys()) + y_col_name].dropna().reindex()
    for t_name, t_excl in dummy_vars.items():
        ...
        del t_excl, t_name
        df_logistic = pd.concat([df_logistic, t_df_dummies], axis=1).drop(
            columns=t_cols_to_drop)
...
print('Regression data frame has %d observations and %d variables (incl. dependent)' %
      df_logistic.shape)
...
if add_const:
    ...
    sm_result = sm.Logit(
        endog=df_logistic[y_col_name], exog=df_logistic[X_col_name].astype(float)).fit()
...
def get_select_hands(df_f, true_cond_cols_f):
    ...
    for t_g_num in t_df['game'].unique():
        select_hands_dict_f.update({str(t_g_num): [str(
            x) for x in t_df.loc[t_df['game'] == t_g_num, 'hand'].unique()]})
...
for p in players:
    ...
    # excl_cond2 = df.big_blind  # less likely to fold if already have money in the pot
    # excl_cond4 = df.premium_hole  # got lucky with good hole cards no one folds
    t_select_hands_prev_loss = get_select_hands(df.loc[df.player == p.name], true_cond_cols_f={
                                                'prev_outcome_loss': True, 'small_blind': False, 'big_blind': False, 'premium_hole': False})
    t_select_hands_no_prev_loss = get_select_hands(df.loc[df.player == p.name], true_cond_cols_f={
                                                   'prev_outcome_loss': False, 'small_blind': False, 'big_blind': False, 'premium_hole': False})
    ...
    looseness_comp.update({p.name: {'p1': t_p1, 'n1': n1_success,
                                    'p2': t_p2, 'n2': n2_success, 'z': t_z, 'pval': t_p}})

    del p, t_p1, t_p2, t_z, t_p, n1_success, n1, n2_success, n2, t_select_hands_no_prev_loss, t_select_hands_prev_loss

print("Partitioning samples on %s and success event = '%s'." %
      ('prev_outcome_loss', 'voluntarily played hand (looseness)'))
print("Test statistic z = (p1 - p2)/stdev and p1 is %s is True" %
      ('prev_outcome_loss'))
...
# output transformed data to csv file
